...
/**
 * Set coin balance command.
 */
public class SetCoinCommand extends MiraeCommand {
    /**
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String @NotNull [] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(Component.text("/setcoin 대상 금액").style(MX.STYLE_WARNING));
            return false;
        }
        ...
        long coins = Math.round(MX.parseDouble(args[1]));

        long after = account.modifyCoinBalance(ignored -> coins, EconomyCause.ADMIN_COMMAND);
