 ...
 */
public class SetMoneyCommand extends MiraeCommand {
    /**
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String @NotNull [] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(Component.text("/setmoney 대상 금액").style(MX.STYLE_WARNING));
            return false;
        }
        ...
        double money = MX.parseDouble(args[1]);

        double after = account.modifyBalance(ignored -> money, EconomyCause.ADMIN_COMMAND);
